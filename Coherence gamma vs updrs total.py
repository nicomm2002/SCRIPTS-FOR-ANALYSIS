...
# *** MODIFICACIÓN: Nueva ruta para los datos de coherencia de gamma ***
COHERENCE_DATA_DIR = Path(r"C:\Users\usuario\Desktop\PYTHON SCRIPTS\coherencegamma")

# Archivos de UPDRS
UPDRS_OFF_FILE = BIDS_ROOT_RAW_DATA / "participants_updrs_off.tsv"
UPDRS_ON_FILE = BIDS_ROOT_RAW_DATA / "participants_updrs_on.tsv"

OUTPUT_PLOTS_DIR = Path(r"C:\Users\usuario\Desktop\NICOLAS MARTINEZ- INSTITUTO CAJAL (TFM)\PLOT_RESULTS")
OUTPUT_PLOTS_DIR.mkdir(parents=True, exist_ok=True)

# Bandas de frecuencia que estamos analizando (¡DEFINICIÓN DE BANDAS!)
# *** MODIFICACIÓN: Solo la banda 'gamma' ***
BANDS_FREQS = {
    'gamma': (30, 100)
}
# La lista de bandas para los análisis (claves del diccionario anterior)
...
def load_coherence_data(data_dir, bands_freqs):
    ...
    # *** PATRÓN CORREGIDO ***
    # Espera un base_name como: sub-0cGdk9_ses-PeriOp_task-HoldL_acq-MedOff_run-1
    base_name_pattern = re.compile(r'sub-(?P<subject>[a-zA-Z0-9]+)_.*?acq-(?P<condition>MedOff|MedOn)_run-\d+$')
    ...
    for subject_folder in all_subject_folders:
        subject_id_raw = subject_folder.name # Ej: sub-0cGdk9
        subject_id = subject_id_raw.replace('sub-', '') # Limpiar para el merge posterior

        # Obtener todos los archivos _coh y _ind.pkl en esta carpeta de sujeto
        ...
        # Crear diccionarios para un acceso fácil por base_name
        # El base_name es el nombre del archivo sin la extensión _coh o _ind.pkl
        ...
        for base_name in coh_files_dict.keys():
            ...
            pkl_path = pkl_files_dict.get(base_name) # Usar .get para evitar KeyError si falta el pkl
            ...
            if not match:
                print(f"DEBUG: Base_name '{base_name}' NO COINCIDE con el patrón de extracción de sujeto/condición. Saltando.")
                continue
            
            extracted_condition = match.group('condition')

            try:
                # 1. Cargar el objeto de conectividad
                coh = mne_connectivity.read_connectivity(coh_path)
                
                # 2. Cargar la información adicional del .pkl
                with open(pkl_path, 'rb') as f:
                    coh_info = pickle.load(f)

                # Asegurarse de que tenemos los datos de coherencia y frecuencias
                coh_data = coh.get_data() # Esto es (n_connections, n_freqs)
                ...
                # Asegurarse de que coh_info tiene los campos esperados
                ...
                # Reshape los datos para promediar correctamente
                # Asegurarse de que el número de conexiones coincida con n_lfp * n_meg
                ...
                # 3. Calcular la coherencia media para CADA BANDA
                for band_name, (fmin, fmax) in bands_freqs.items():
                    ...
                    if len(band_idx) == 0:
                        ...
                        mean_coh_band = np.nan # Usar NaN si no hay datos para esa banda
                    else:
                        # Promedio en frecuencia para cada conexión LFP-MEG
                        coh_band_per_connection = np.mean(coh_data_reshaped[:, :, band_idx], axis=2)
                        mean_coh_band = np.mean(coh_band_per_connection) # Promedio global de la banda
    ...
    print(f"\nCargados {len(df_coh)} registros de coherencia.")
    return df_coh
...
def prepare_merged_data():
    ...
    df_coh = load_coherence_data(COHERENCE_DATA_DIR, BANDS_FREQS) # Pasamos el diccionario de bandas
    
    if df_coh.empty:
        print("No se pudieron cargar los datos de coherencia. Terminando.")
        sys.exit(1)

    # Pivotear la tabla de coherencia para tener una columna por banda y condición
    # Antes de pivotar, si hay múltiples registros por sujeto-condición-banda (ej. varias tareas), promediar.
    # Esto es crucial si tienes multiples tareas por sujeto y condición, y solo quieres una coherencia por sujeto/condición.
    df_coh_agg = df_coh.groupby(['subject', 'condition', 'band'])['coherence_mean'].mean().reset_index()

    # *** ADVERTENCIA RESUELTA: observed=False añadido a pivot_table ***
    df_coh_pivot = df_coh_agg.pivot_table(index=['subject', 'condition'], 
                                          columns='band', 
                                          values='coherence_mean',
                                          observed=False).reset_index() # Añadido observed=False
    df_coh_pivot.columns.name = None # Limpiar el nombre del índice
    
    # Unir datos de UPDRS y coherencia
    merged_data = pd.merge(df_updrs, df_coh_pivot, on=['subject', 'condition'], how='inner')
    
    # Calcular Deltas (MedOn - MedOff)
    delta_data = []
    ...
    for subject in merged_data['subject'].unique():
        ...
        # Obtener los valores de coherencia para cada banda
        coh_vals = {
            band: {
                'MedOff': data_sub[(data_sub['condition'] == 'MedOff')][band].values,
                'MedOn': data_sub[(data_sub['condition'] == 'MedOn')][band].values
            } for band in BANDS_FREQS.keys() # Usar las bandas definidas
        }
        
        # Solo procesar si tenemos todos los datos necesarios para MedOff y MedOn
        # y si los arrays tienen un solo elemento (lo que se espera después del groupby y pivot)
        all_present = True
...
def save_plot(fig, filename, show=True): # Añadido el parámetro 'show'
    ...
    plt.close(fig) # Cierra la figura después de guardar/mostrar
...
def plot_updrs_violin_boxplot(data_df):
    ...
    # La advertencia de FutureWarning sobre 'color='white'' se ignora por ahora
    ...
    # *** CORRECCIÓN: Se mantiene el código anterior, pero la leyenda se controla
    #    para evitar conflictos/errores. Si Seaborn ya la crea, esta parte la ajusta.
    handles, labels = ax.get_legend_handles_labels()
    if len(handles) > 2: # Si hay más de 2 entradas (ej. de stripplot), simplificar
        ax.legend(handles[:2], labels[:2], title='Condición')
    else: # Si solo hay 2 o menos, y Seaborn ya la creó, no es necesario hacer nada más
        if ax.legend_ is not None:
            ax.legend_.remove() # Solo intenta remover si existe

    save_plot(fig, 'UPDRS_MedOff_MedOn_ViolinBoxplot.png', show=True) # Mostrar este gráfico

def plot_coherence_distributions(data_df, band):
    ...
    ax_hist.set_title(f'Histograma de Coherencia Media {band.capitalize()} por Condición')
    ax_hist.set_xlabel(f'Coherencia Media {band.capitalize()} (LFP-MEG)')
    ax_hist.set_ylabel('Frecuencia')
    ax_hist.grid(axis='y', linestyle='-', alpha=0.7)
    save_plot(fig_hist, f'Histograma_Coherencia_{band.capitalize()}.png', show=True) # Mostrar este gráfico

    fig_box, ax_box = plt.subplots(figsize=(10, 7))
    sns.boxplot(data=df_band, x='condition', y=band, hue='condition', # Hue crea la leyenda
                palette={'MedOff': 'red', 'MedOn': 'blue'}, ax=ax_box,
                boxprops=dict(edgecolor='black'), medianprops=dict(color='black'),
                whiskerprops=dict(color='black'), capprops=dict(color='black'))
    ax_box.set_title(f'Comparación de Coherencia Media {band.capitalize()} entre MedOff y MedOn (Boxplot)')
    ...
    # *** CORRECCIÓN: Eliminada llamada redundante a ax_box.legend() ***
    save_plot(fig_box, f'Boxplot_Coherencia_{band.capitalize()}.png', show=True) # Mostrar este gráfico

    fig_violin, ax_violin = plt.subplots(figsize=(10, 7))
    sns.violinplot(data=df_band, x='condition', y=band, hue='condition', # Hue crea la leyenda
                   palette={'MedOff': 'red', 'MedOn': 'blue'}, inner='quartile', ax=ax_violin,
                   linewidth=1, edgecolor='black')
    ax_violin.set_title(f'Distribución de Coherencia Media {band.capitalize()} entre MedOff y MedOn (Violinplot)')
    ...
    # *** CORRECCIÓN: Eliminada llamada redundante a ax_violin.legend() ***
    save_plot(fig_violin, f'Violinplot_Coherencia_{band.capitalize()}.png', show=True) # Mostrar este gráfico

def plot_coherence_updrs_correlation(df_deltas, merged_data, band):
    ...
    ax_delta.set_title(f'Correlación entre Delta Coherencia {band.capitalize()} y Delta UPDRS')
    ...
    save_plot(fig_delta, f'Correlacion_Delta_Coherencia_{band.capitalize()}_Delta_UPDRS.png', show=True) # Mostrar este gráfico
    ...
    ax_cond.set_title(f'Correlación entre Coherencia Media {band.capitalize()} y Puntuación UPDRS')
    ...
    save_plot(fig_cond, f'Correlacion_Coherencia_{band.capitalize()}_UPDRS_Combinado.png', show=True) # Mostrar este gráfico


# --- 7. Ejecución Principal ---
def main_analysis():
    print("Iniciando carga y preparación de datos...")
    merged_data, df_deltas = prepare_merged_data()
    ...
    # Gráficos de Coherencia por banda
    for band in BANDS:
        plot_coherence_distributions(merged_data, band)
        plot_coherence_updrs_correlation(df_deltas, merged_data, band)
